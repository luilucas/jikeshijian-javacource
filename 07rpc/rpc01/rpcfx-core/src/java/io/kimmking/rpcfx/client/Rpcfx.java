        ...
        static class NettyHttpClient {
            public RpcfxResponse post(RpcfxRequest req, String url) {
                ...
                try {
                    ...
                    ChannelFuture channelFuture = f.channel().writeAndFlush(request, new DefaultChannelPromise(f.channel()));
                    Object result = channelFuture.getNow();
                    String respJson = ((DefaultFullHttpResponse) result).content().toString(CharsetUtil.UTF_8);
                    return JSON.parseObject(respJson, RpcfxResponse.class);
